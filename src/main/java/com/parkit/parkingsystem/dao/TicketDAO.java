...
public class TicketDAO {
    ...
    public Ticket getOpenTicket(String vehicleRegNumber) {
        ...
        try {
            con = dataBaseConfig.getConnection();
            PreparedStatement ps = con.prepareStatement("SELECT * FROM ticket WHERE VEHICLE_REG_NUMBER = ? AND OUT_TIME IS NULL ORDER BY IN_TIME DESC LIMIT 1"
            );
            ps.setString(1, vehicleRegNumber);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                ticket = new Ticket();
                ParkingSpot parkingSpot = new ParkingSpot(rs.getInt("PARKING_NUMBER"), ParkingType.valueOf(rs.getString("TYPE")), false);
                ticket.setParkingSpot(parkingSpot);
            ...
            logger.error("Error fetching open ticket for vehicle", ex);
        } finally {
            dataBaseConfig.closeConnection(con);
        }
        return ticket;
